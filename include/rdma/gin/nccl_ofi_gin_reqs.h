...
public:
	nccl_ofi_rdma_gin_iputsignal_req(
	    ...
	    : /*write_reqs(write_reqs_arg), send_req(send_req_arg),*/ any_reqs_pending(0), gin_comm(gin_comm_arg),
...
#endif
	// Use compare and swap later in case of thread safe
	union {
		bool reqs_pending[MAX_NUM_RAILS + 1];	// write_reqs + send_req
...
public:
	void *comm;
        bool *req_pending_byte;
	int dev;
	uint32_t rank;
	uint16_t msg_seq_num;

	inline void set_info(void *comm_arg, int dev_arg, uint32_t rank_arg, uint16_t msg_seq_num_arg) {
		comm = comm_arg;
		dev = dev_arg;
		rank = rank_arg;
